...
def warm_start_model(checkpoint_path, model, ignore_layers):
    ...
    logging.info("Warm starting model from checkpoint '{}'".format(checkpoint_path))
...
def validate(model, criterion, valset, iteration, batch_size, n_gpus,
    ...
    model.train()
    if rank == 0:
        logging.info("Validation loss {}: {:9f}  ".format(iteration, val_loss))
...
if __name__ == '__main__':
    ...
    logging.info(f"cuDNN Enabled: {hparams.cudnn_enabled}")
    logging.info(f"cuDNN Benchmark: {hparams.cudnn_benchmark}")
